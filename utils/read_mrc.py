...
rec_header_dtd = \
    [
        ...
        ("triangles1", "f4", 2),  # 0,1,2 = original:  3,4,5 = current
        ('nt', 'i2'),
        ('ImgSequence', 'i2'),
        ("triangles2", "f4", 3),

        ("xorg", "f4"),  # Origin of image
        ("yorg", "f4"),  # Origin of image
        ("zorg", "f4"),
...
def read_mrc_with_hd(filename):
    ...
    if header[0][3] > 6:
        ...
        header = np.fromfile(fd, dtype=rec_header_dtd_b, count=1)
        bigFlag = True
    nx, ny, nz, nt = np.int64(header['nx'][0]), np.int64(header['ny'][0]), np.int64(header['nz'][0]), np.int64(
        header['nt'][0])
    ...
    if nz_pt == 1 or nt == 1 or nz_pt.dtype == 0:
        imgrawdata = np.fromfile(fd, read_type)
        img_data = np.ndarray(shape=(nz, nx, ny), dtype=data_type)
        fd.close()
        ...
        imgrawdata = np.fromfile(fd, read_type)
        img_data = np.ndarray(shape=(nt, nz_pt, nx, ny), dtype=data_type)
        fd.close()

        for it in range(nt):
            for iz in range(nz_pt):
                img_data_2d = imgrawdata[nx * ny * (it * nz_pt + iz):nx * ny * (it * nz_pt + iz + 1)]
                img_data[it, iz, :, :] = img_data_2d.reshape(nx, ny, order='F')
...
def write_mrc(filename, img_data, header):
    ...
    if header.dtype == rec_header_dtd_b: header = header.astype(rec_header_dtd)
    ...
    imgrawdata = np.ndarray(shape=(nx * ny * nz), dtype=img_data.dtype)
    ...
    if img_data.ndim == 3:
        ...
        for it in range(nt):
            for iz in range(nz_pt):
                imgrawdata[nx * ny * (it * nz_pt + iz):nx * ny * (it * nz_pt + iz + 1)] = \
                    img_data[it, iz, :, :].reshape(nx * ny, order='F')
...
if __name__ == '__main__':
    # filename = '/mnt/Public/NL-SIM2/20230517-NL/cos7-2xkohinoor2.0-paxillian/c1-low-5-8-15p-5ms-high-5-7-35p-5ms-141NA-5scyc-7cycles_20230517_152753/TIRF488-5ang5pha-4step_cam1_step2_001.mrc'
    filename = r'V:/NL-SIM3/3D NL-SIM/20231026-3DNL/1xkohinoor2.0-lifeact-zstep84nm/timepoints/c5_20231026_191846/3D-488-NL_cam1_step1_001_SIrecon.mrc'
    # filename = 'Z:/NL-SIM2/20230517-NL/cos7-2xkohinoor2.0-paxillian/c1-low-5-8-15p-5ms-high-5-7-35p-5ms-141NA-5scyc-7cycles_20230517_152753/TIRF488-5ang5pha-4step_cam1_step2_001.mrc'
    ...
    savename = r'F:\chy\NLSIM-project\img\test_3d_origin_endian.mrc'
