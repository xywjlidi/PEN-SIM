...
from utils.read_utils import load_recon_no_negative, load_recon_both, \
    is_specify_file, prctile_norm, load_tif_img_origin
...
if __name__ == "__main__":
    ...
    filelist = glob.glob(r'../demo_data/100nm_beads.mrc')
    headerlist = glob.glob(r'../demo_data/100nm_beads.mrc')
    # TODO: file and header must be matched
    assert len(filelist) == len(headerlist)
    ...
    with torch.no_grad():
        ...
        for (f, header_file) in zip(filelist,headerlist):
            with timeblock("**********************************\n time for localizating %s : " % f):
                ...
                # load tif
                    ...
                    continue
                _, header = read_mrc_with_hd(header_file)
                ...
                write_mrc(f[0:-4] + "_" + env + "_output_" + str(start) + "_step" + str(
                                                     interval) + ".tif", np.float32(result), header)
