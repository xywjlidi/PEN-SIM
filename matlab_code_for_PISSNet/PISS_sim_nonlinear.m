...
file_path = '.\data\denoising\SKL_NL';

file_list = dir(file_path);
save_dir = file_dir;

parfor j = 3:size(file_list,1)
%     for i = 3:4
    file_name = file_list(j).name;
    if contains(file_name,'view')
        continue;
    elseif contains(file_name,'SIrecon')
        continue;
    elseif contains(file_name,'tif')
        continue;
    end
    ...
    for view_id = 1: 1: view_count
        ...
        save_file_name = [file_name(1:end-4),'-view-',num2str(view_id),'.mrc'];
        handle = fopen([save_dir,'/',save_file_name],'w+');
        handle = XxWriteMRC_SmallEndian(handle, data_view(:), header_out);
        fclose(handle);
    end
