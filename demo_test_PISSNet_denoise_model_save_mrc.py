...
if __name__ == "__main__":
    ...
    # 3D single structure(488 channel):
    #           "lifeact-3D-Neighbor2Neighbor"
    # TODO: change model
    env = "lifeact"


    # TODO: change input path
    # only support MRC file
    # img_pattern in unix file filename pattern matching
    filelist = glob.glob(r"../demo_data/Lifeact.mrc")
    ...
    with torch.no_grad():
        ...
        for f in filelist:
            with timeblock("**********************************\n time for denoising %s : " % f):
                ...
                save_name = f[0:-4] + "_denoise_" + str(model) + ".mrc"
                if len(save_name.split('\\')[-1]) > 40:
                    save_name = f[0:-4] + "_denoise_" + str(env) + ".mrc"
                ...
                if env.find("3D") == -1:
                    if img.ndim == 2:
                        ...
                        # save output
                        write_mrc(save_name, np.float32(restored_t)[np.newaxis,:,:], header)
                        ...
                        # save output
                        write_mrc(save_name, np.float32(restored_t), header)
                    elif img.ndim == 4:
                        ...
                        for t in range(nt_aft_interval):
                            for iz in range(nz):
                                ...
                                result_t = kernel_test_model_2d(Net, device, clip, kernel=kernel2D, overlap=padding2D, upfactor=1)
                                result_t = result_t.cpu().numpy().squeeze()
                                restored_t[t,iz] = result_t

                        # save output
                        write_mrc(save_name, np.float32(restored_t), header)
                    else:
                        print("data dimension does not correspond to 2, 3 or 4")
                else:
                    if img.ndim == 3:
                        nz, nx, ny = img.shape
                        if test_interval != 1:
                        ...
                        for t in range(nt_aft_interval):
                            clip = img[t * test_interval]
                            result_t = kernel_test_model_3d(Net, device, clip, kernel=kernel3D,
                                                            padding=padding3D,
                                                            upfactor=1)
                            result_t = result_t.cpu().numpy().squeeze()
                            restored_t[t] = result_t
                        ...
                        print("data dimension does not correspond to 3 or 4")
